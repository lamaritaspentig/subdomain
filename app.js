...
function renderDashboardStats() {
  ...
  document.getElementById('statProxied').textContent = state.records.filter(r => r.proxied).length;
  document.getElementById('statDnsOnly').textContent = state.records.filter(r => !r.proxied).length;
}
...
async function confirmDelete() {
  ...
  const record = state.deleteRecord;
  closeModal('deleteModal');
  showLoading('Menghapus DNS...');

  try {
    await gas('deleteDnsRecord', state.activeZoneId, record.id);
    state.deleteRecord = null;
    toast('DNS record berhasil dihapus.', 'success');
    await loadRecords();
  } catch (err) {
    toast(err.message, 'error');
  } finally {
...
async function testConnection() {
  ...
  try {
    ...
    toast(`Cloudflare terhubung. ${result.zoneCount} zone tersedia.`, 'success');
  } catch (err) {
    renderConnectionStatus(false);
    toast(err.message, 'error');
  } finally {
