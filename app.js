...
window.addEventListener('message', event => {
  ...
  if (!isAppsScriptResponseOrigin(event.origin)) return;
...
function gas(method, ...args) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      iframe.remove();
      reject(new Error('Backend Apps Script tidak merespons dalam 30 detik.'));
    }, 30000);
...
async function initialize() {
  ...
  try {
    ...
    state.tunnels = Array.isArray(initial.tunnels) ? initial.tunnels : [];
    renderConnectionStatus(initial.configured !== false);

    populateZones();
    populateTunnels();
    document.getElementById('accountStatus').textContent = initial.accountId ? 'Terdeteksi' : 'Belum tersedia';
    document.getElementById('tunnelPermissionStatus').textContent = initial.tunnelAccess ? 'Siap' : 'Periksa permission token';
    ...
    const chosen = state.zones.some(z => z.id === savedZone) ? savedZone : state.zones[0].id;
    ...
    await loadRecords();
    if (state.activeTunnelId) await loadPublishedApps();
    else renderPublishedApps();
  } catch (err) {
...
function populateTunnels() {
  ...
  const html = state.tunnels.length
    ...
    : '<option value="">Tidak ada remote-managed tunnel</option>';
...
async function savePublishedApp(e){
  ...
  try{if(state.editPublished){await gas('updatePublishedApplication',tunnelId,state.editPublished.index,payload);toast('Published Application berhasil diperbarui.','success');}else{await gas('createPublishedApplication',tunnelId,payload);toast('Published Application berhasil dibuat.','success');}state.editPublished=null;closeModal('publishedModal');await loadPublishedApps();await loadRecords();}catch(err){toast(err.message,'error');}finally{hideLoading();}
}
...
async function confirmDelete() {
  if (state.deletePublished) {
    ...
    try { await gas('deletePublishedApplication',state.activeTunnelId,route.index); state.deletePublished=null; toast('Published Application berhasil dihapus.','success'); await loadPublishedApps(); await loadRecords(); }
...
async function testConnection() {
  ...
  try {
    ...
    toast(`Cloudflare terhubung. ${result.zoneCount} zone dan ${result.tunnelCount} tunnel tersedia.`, 'success');
  } catch (err) {
    renderConnectionStatus(false);
    toast(err.message, 'error');
  } finally {
